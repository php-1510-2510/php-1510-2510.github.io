...
$$Pr(3\le X\le 7)= PR(X\le 7) - Pr(X\le 3)$$
    - We recognize this as the CDF function which in R is the `dbinom()`. 
    ```{r}
    dbinom(7, 10, 0.8) - dbinom(3, 10, 0.8)
    ```
...
2. Suppose you want to repeat this experiment 100 times over in a population that have a 75% chance of survival after 5 years.
    ...
    mean(data2)
    ```
    - We can see that we have a slightly higher value of `r 7.68` compared the the theoretical mean of 7.5. 
...
- We can actually plot this out:

```{R}
juries <- binom.sim(1000, 12,0.1)
ggplot(data=NULL, aes(x=juries)) + 
  geom_bar()
```
